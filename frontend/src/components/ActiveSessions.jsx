        ...
        <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
          {isLoading ? (
            ...
            sessions.map((session) => {
              // Calculate participant count for 4-person sessions
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 text-sm opacity-80">
                          ...
                          {isFull && !userInSession ? (
                            ...
                            <span className="badge badge-info badge-sm">JOINED</span>
                          ) : (
                        ...
                        {session.participants && session.participants.length > 0 && (
                          <div className="mt-2 text-xs opacity-60">
                            ...
                            {session.participants.slice(0, 2).map((p, index) => (
                              <span key={p.user?._id || index}>
                                {p.user?.name}{index < Math.min(session.participants.length - 1, 1) ? ', ' : ''}
                              </span>
                            ))}
                            {session.participants.length > 2 && (
                              <span> +{session.participants.length - 2} more</span>
                            )}
                          </div>
                        )}
                      </div>
